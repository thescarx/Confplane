...
function AssignArticles() {
  ...
  useEffect(() => {
    axios
    .get(
      "http://192.168.8.101:8000/articles/listforchairman/path/?conference_id="+id+"&status=accepted to review"
    )
    .then((response) => {
  ...
  const handleDeleteRev = (id1, id2) => {
     axios
      .post(
        "http://192.168.8.101:8000/articles/remove_reviewer_from_article_reviewers",
        {
          article: id1,
          user: id2,
        }
